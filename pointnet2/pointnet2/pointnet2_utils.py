...
class FurthestPointSampling(Function):
    ...
    def forward(ctx, xyz: torch.Tensor, npoint: int) -> torch.Tensor:
        ...
        temp = torch.full((B, N), 1e10, device=xyz.device, dtype=xyz.dtype)
...
class ThreeNN(Function):
    ...
    def forward(ctx, unknown: torch.Tensor, known: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        B, N, _ = unknown.size()
        m = known.size(1)
        dist2 = torch.empty((B, N, 3), device=unknown.device, dtype=unknown.dtype)
...
class ThreeInterpolate(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        ...
        output = torch.empty((B, c, n), device=features.device, dtype=features.dtype)
...
class BallQuery2(Function):
    ...
    @staticmethod
    def forward(ctx, nsample: int, radius_size : torch.Tensor  , xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        """
        :param ctx:
        :param radius: float, radius of the balls
        :param nsample: int, maximum number of features in the balls
        ...
        assert new_xyz.is_contiguous()
        assert xyz.is_contiguous()
        assert radius_size.is_contiguous()

        B, N, _ = xyz.size()
        npoint = new_xyz.size(1)
        idx = torch.zeros((B, npoint, nsample), device=xyz.device, dtype=torch.int32)

        pointnet2.ball_query2_wrapper(B, N, npoint, nsample, radius_size , new_xyz, xyz, idx)
        return idx
...
class QueryAndGroup2(nn.Module):
    ...
    def forward(self, radius_size : torch.Tensor , xyz: torch.Tensor, new_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[torch.Tensor]:
        """
        :param xyz: (B, N, 3) xyz coordinates of the features
        :param new_xyz: (B, npoint, 3) centroids
        :param features: (B, C, N) descriptors of the features
        ...
        idx = ball_query2(self.nsample, radius_size, xyz, new_xyz)
        xyz_trans = xyz.transpose(1, 2).contiguous()
        grouped_xyz = grouping_operation(xyz_trans, idx)  # (B, 3, npoint, nsample)
        grouped_xyz -= new_xyz.transpose(1, 2).unsqueeze(-1)
